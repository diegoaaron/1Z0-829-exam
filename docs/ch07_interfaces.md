...
var s = Season.VERANO;
System.out.println(s); // VERANO
System.out.println(Season.VERANO); // VERANO
System.out.println(s == Season.VERANO); // true
...
En un switch con enumeraciones, Java va a inferir que estás usando una enumeración por lo que se puede usar directamente el nombre de la enumeración.
...
public enum Season {
    WINTER("Low"), SPRING("Medium"), SUMMER("High"), AUTUMN("Medium"); // define los valores de la enumeración y termina con `;` ya que hay más información.
    
    private final String expectedVisitors; // no se puede modificar, ya que las enumeraciones son inmutables y solo debe haber una en la JVM.
...
* Si bien el compilador puede aplicar reglas sobre la conversión a tipos no relacionados para las clases, no siempre puede hacer lo mismo para las interfaces. 
* Recuerde que las instancias admiten herencia múltiple, lo que limita lo que el compilador puede razonar sobre ellas. 
* Si bien una clase determinada puede no implementar una interfaz, es posible que alguna subclase si la implemente. 
...
```java
// En este programa, se creó un objeto 'Wolf' y luego se asigna a un tipo de referencia 'Wolf'.
// Con las interfaces, el compilador tiene una capacidad limitada para aplicar muchas reglas porque, aunque un tipo 
// de referencia no implemente una interfaz, una de sus subclases si podría. 
// Por lo tanto, permite la conversión no válida al tipo de referencia 'Dog', aunque 'Dog' y 'Wolf' no estén relacionados. 
// Aunque el código compile, todavía lanza una 'ClassCastException' en tiempo de ejecución. 
...
* Dejando de lado esta limitación, el compilador puede aplicar una regla sobre la conversión de interfaces. 
* El compilador no permite una conversión de una referencia de interfaz a una referencia de objeto si el tipo de objeto no puede implementar la interfaz (por ejemplo si la clase está marcada como final).
* Por ejemplo, si la clase `Wolf` está marcada como `final`, la línea 8 ya no se compila. El compilador reconoce que no hay subclases posibles de 'Wolf' capaces de implementar la interfaz 'Dog'.
